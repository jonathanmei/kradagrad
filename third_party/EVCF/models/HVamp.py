...
from utils.nn import GatedDense, NonLinear, he_init
...
class VAE(Model):
    def __init__(self, args):
        ...
        self.q_z2_logvar = NonLinear(
            self.args.hidden_size,
            self.args.z2_size,
            activation=nn.Hardtanh(min_val=-12.0, max_val=4.0),
        )
        ...
        if self.args.input_type == "multinomial":
            self.p_x_mean = NonLinear(
                self.args.hidden_size, np.prod(self.args.input_size), activation=None
            )
    ...
    def calculate_loss(self, x, beta=1.0, average=False):
        ...
        if self.args.input_type == "binary":
            ...
            RE = log_Softmax(
                x, x_mean, dim=1
            )  #! Actually not Reconstruction Error but Log-Likelihood
