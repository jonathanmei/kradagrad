...
# Uses largest k eigenvalues of VSVT + GGT
...
class BSPreconditioner(Preconditioner):
    ...
    def __init__(self, var, hps, debug=False, **unused):
        ...
        if rank < 1:
            ...
            self.ss = []  # store eigvals
            self.statistics_buffer = []  # accum G G^T
            self.Vs = []  # store eigvectors
            self.hadamard_factor = []  # store S = diag(s)
        else:
            ...
            self.hadamard_factor = [torch.ones(tuple(r for r in self.rs), device=device, dtype=precision)]
    ...
    def compute_preconditioners(self, **kwargs):
        ...
        w1 = self._hps.beta2
        w2 = 1.0 if w1 == 1.0 else (1.0 - w1)
        for i, (r, s, stat_accum, V) in enumerate(zip(self.rs, self.ss, self.statistics_buffer, self.Vs)):
            stat_new = (V @ (s[:, None] * V.T)).mul(w1).add(stat_accum, alpha=w2)
            try:
                # will fail if r is too large relative to shape of stat_new, among other reasons
                s_new, V_new = torch.lobpcg(stat_new, r)
            except:
                s_new, V_new = torch.linalg.eigh(stat_new)
                s_new, V_new = s_new[-r:], V_new[:, -r:]
            self.Vs[i] = V_new
            self.ss[i] = s_new
        
        # build kronecker product of eigvals
        slc = (slice(None), ) + (None, ) * (len(self.ss) - 1)
        S = self.ss[0][slc]
        for i, s in enumerate(self.ss[1:]):
            slc = (None, ) * (i + 1) + (slice(None), ) + (None, ) * (len(self.ss) - 2 - i)
            S = S * s[slc]
        self.hadamard_factor[0] = (S + eps) ** (-1/exp) - eps ** (-1/exp)
    ...
    def preconditioned_grad(self, grad, **kwargs):
        ...
        for j, V in enumerate(self.Vs):
            ...
            precond_grad = pg_
        precond_grad *= self.hadamard_factor[0]
        for j, V in enumerate(self.Vs):
