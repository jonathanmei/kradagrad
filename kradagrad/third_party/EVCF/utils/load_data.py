...
def load_ml20m(args, **kwargs):
    ...
    with open(
        os.path.join(
            "/home/lukewalters/code/EVCF", "datasets", "ML_20m", "unique_sid.txt"
        ),
        "r",
    ) as f:
        for line in f:
    ...
    x_train = load_train_data(
        os.path.join("/home/lukewalters/code/EVCF", "datasets", "ML_20m", "train.csv")
    )
    np.random.shuffle(x_train)
    x_val_tr, x_val_te = load_tr_te_data(
        os.path.join(
            "/home/lukewalters/code/EVCF", "datasets", "ML_20m", "validation_tr.csv"
        ),
        os.path.join(
            "/home/lukewalters/code/EVCF", "datasets", "ML_20m", "validation_te.csv"
        ),
    )

    x_test_tr, x_test_te = load_tr_te_data(
        os.path.join(
            "/home/lukewalters/code/EVCF", "datasets", "ML_20m", "test_tr.csv"
        ),
        os.path.join(
            "/home/lukewalters/code/EVCF", "datasets", "ML_20m", "test_te.csv"
        ),
    )
    ...
    test = data_utils.TensorDataset(
        torch.from_numpy(x_test_tr).float(), torch.from_numpy(x_test_te)
    )
...
def load_netflix(args, **kwargs):
    ...
    np.random.shuffle(x_train)
    x_val_tr, x_val_te = load_tr_te_data(
        os.path.join("datasets", "Netflix", "validation_tr.csv"),
        os.path.join("datasets", "Netflix", "validation_te.csv"),
    )

    x_test_tr, x_test_te = load_tr_te_data(
        os.path.join("datasets", "Netflix", "test_tr.csv"),
...
def load_pinterest(args, **kwargs):
    ...
    np.random.shuffle(x_train)
    x_val_tr, x_val_te = load_tr_te_data(
        os.path.join("datasets", "Pinterest", "validation_tr.csv"),
        os.path.join("datasets", "Pinterest", "validation_te.csv"),
    )

    x_test_tr, x_test_te = load_tr_te_data(
        os.path.join("datasets", "Pinterest", "test_tr.csv"),
...
def load_melon1d(args, **kwargs):
    ...
    unique_sid = list()
    with open(
        os.path.join("datasets", "Melon/v2-mat-mm-1d/pro_sg", "unique_sid.txt"), "r"
    ) as f:
        for line in f:
            unique_sid.append(line.strip())
    ...
    # train, validation and test data
    x_train = load_train_data(
        os.path.join("datasets", "Melon/v2-mat-mm-1d/pro_sg", "train.csv")
    )
    np.random.shuffle(x_train)
    x_val_tr, x_val_te = load_tr_te_data(
        os.path.join("datasets", "Melon/v2-mat-mm-1d/pro_sg", "validation_tr.csv"),
        os.path.join("datasets", "Melon/v2-mat-mm-1d/pro_sg", "validation_te.csv"),
    )

    x_test_tr, x_test_te = load_tr_te_data(
        os.path.join("datasets", "Melon/v2-mat-mm-1d/pro_sg", "test_tr.csv"),
...
def load_melon8d(args, **kwargs):
    ...
    unique_sid = list()
    with open(
        os.path.join("datasets", "Melon/v2-mat-mm-8d/pro_sg", "unique_sid.txt"), "r"
    ) as f:
        for line in f:
            unique_sid.append(line.strip())
    ...
    # train, validation and test data
    x_train = load_train_data(
        os.path.join("datasets", "Melon/v2-mat-mm-8d/pro_sg", "train.csv")
    )
    np.random.shuffle(x_train)
    x_val_tr, x_val_te = load_tr_te_data(
        os.path.join("datasets", "Melon/v2-mat-mm-8d/pro_sg", "validation_tr.csv"),
        os.path.join("datasets", "Melon/v2-mat-mm-8d/pro_sg", "validation_te.csv"),
    )

    x_test_tr, x_test_te = load_tr_te_data(
        os.path.join("datasets", "Melon/v2-mat-mm-8d/pro_sg", "test_tr.csv"),
