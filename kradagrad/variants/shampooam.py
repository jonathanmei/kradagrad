""" Actually arithmetic mean. real harmonic is in _real
"""
...
class ShampooAMPreconditioner(Preconditioner):
    ...
    for j in range(self._len_partitioned_grads):
      ...
      for i in range(rank):
        ...
        self.eigs[j] = self.eigs[j] + (
          torch.maximum(L, eps * torch.ones(1, device=device))
        )[tuple(expands)]
...
class ShampooAM(Shampoo):
  ...
  def init_var_state(self, var, state):
    ...
    state[PRECONDITIONER] = ShampooAMPreconditioner(var, self.hps)
